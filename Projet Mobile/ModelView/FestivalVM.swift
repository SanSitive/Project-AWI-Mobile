...
struct FestivalDTO : Decodable{
  ...
  static func festivalDTO2Festival(data: [FestivalDTO]) -> [FestivalVM]?{
    var festivals: [FestivalVM] = []
    for fdata in data{
      guard (fdata.id != nil) else{
          return nil
      }
      let id : Int = fdata.id!
      let festival : FestivalVM = FestivalVM(id: id, nom: fdata.nom, annee: fdata.annee, isActive: fdata.isActive)
